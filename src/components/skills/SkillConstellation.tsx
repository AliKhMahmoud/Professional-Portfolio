...
function clusterNodePositions(
  ...
  if (count === 0) return [];
  const effectiveRadius = count > 1 ? Math.max(radius, (65 * count) / (2 * Math.PI)) : 0;
...
export const SkillConstellation: React.FC<SkillConstellationProps> = ({ skills, activeCategory }) => {
  ...
  return (
    <div className="relative w-full overflow-hidden select-none">
      {/* SVG Canvas */}
      <svg
        className="w-full h-auto max-h-[620px] block"
        viewBox={`0 0 ${w} ${h}`}
        ...
        {clusterNodes.map(({ cluster, nodes }) => {
          ...
          return nodes.map(({ skill, x, y }) => (
            <line
              ...
              strokeWidth={hoveredId === skill.id ? 1.5 : 0.8}
              opacity={vis ? (hoveredId === skill.id ? 0.8 : 0.25) : 0.05}
        ...
        {clusterNodes.map(({ cluster }) => {
          ...
          return (
            <g key={`center-${cluster.category}`} opacity={vis ? 1 : 0.15} className="transition-opacity duration-500">
              {/* Outer glow ring */}
              <circle cx={cx} cy={cy} r={22} fill={cluster.accentColor} opacity={0.08} />
              <circle cx={cx} cy={cy} r={16} fill={cluster.accentColor} opacity={0.15}
                filter={`url(#glow-${cluster.category})`} />
              {/* Inner dot */}
              <circle cx={cx} cy={cy} r={6} fill={cluster.accentColor} opacity={0.9} />
              {/* Label */}
              <text
                x={cx} y={cy - 28}
                textAnchor="middle"
        ...
        {allNodes.map(({ skill, x, y, cluster }) => {
          ...
          return (
            ...
            >
              ...
              {isHov && (
                <circle r={nodeR + 10} fill={skill.color} opacity={0.12} filter="url(#node-glow)" />
              )}
              {/* Pulsing ring */}
              <circle
                r={nodeR + 6}
              ...
              <circle r={nodeR} fill={skill.color} opacity={isHov ? 0.15 : 0.05} className="transition-all duration-300" />

              {/* Skill name label below */}
              <text
                y={nodeR + 14}
                textAnchor="middle"
                fill={isHov ? skill.color : '#94a3b8'}
                fontSize={isHov ? 10 : 9}
                fontWeight={isHov ? 700 : 500}
                fontFamily="system-ui, sans-serif"
                className="transition-all duration-300 select-none"
              >
                {skill.name.length > 12 ? skill.name.substring(0, 11) + '…' : skill.name}
              </text>

              {/* Percentage badge on hover */}
              {isHov && (
                <g transform={`translate(${nodeR - 2}, ${-nodeR - 2})`}>
                  <circle r={11} fill={skill.color} opacity={0.95} />
                  <text
                    textAnchor="middle"
                    y={4}
                    fill="white"
              ...
              <foreignObject x={-12} y={-12} width={24} height={24} style={{ overflow: 'visible' }}>
                ...
                >
                  <SkillIcon icon={skill.icon} className="w-5 h-5" />
                </div>
              </foreignObject>
            </g>
          );
